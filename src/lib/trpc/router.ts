...
import type {
	...
	DatasetInfo,
	ModelInfo
} from '$lib/server/helpers/apiTypes'
...
export const graphs = t.router({
	testGraph: t.procedure.query(async () => {
...
export const nodes = t.router({
	...
	newModelNode: t.procedure
		...
		.mutation(async ({ input }) => {
			let x: APINode = await createModelNode(input.modelInfo, input.graphId, input.position)
	...
	newDatasetNode: t.procedure
		...
		.mutation(async ({ input }) => {
			let x: APINode = await createDatasetNode(input.datasetInfo, input.graphId, input.position)
...
export const edges = t.router({
	newEdge: t.procedure
		...
		.mutation(async ({ input }) => {
			let x: APIEdge = await createEdge(
				input.sourceNodeId,
				input.targetNodeId,
